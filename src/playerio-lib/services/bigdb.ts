...
import { ValueType, EntryType, LockType } from '../constants.ts'
import { compareForChanges } from '../utilities/util.ts'
...
export default class BigDB {
  ...
  createObject(table: string, key: string, obj: any): Promise<DatabaseObject> {
    ...
    return this.channel.createObjects([{ key, table, properties }], false).then((results) => {
      ...
      .then(
        (result) =>
          new DatabaseObject(
            this,
            'PlayerObjects',
            result.playerobject.key,
            result.playerobject.version,
            true,
            result.playerobject.properties,
          ),
      )
  }
  ...
  load(table: string, key: string): Promise<DatabaseObject | null> {
    return this.channel.loadObjects([{ table: table, keys: [key] }]).then((results) => {
      if (results.objects.length == 1) {
        return results.objects[0] == null
          ? null
          : new DatabaseObject(
              this,
              table,
              results.objects[0].key,
              results.objects[0].version,
              false,
              results.objects[0].properties,
            )
      } else {
  ...
  loadKeys(table: string, keys: string[]): Promise<(DatabaseObject | null)[]> {
    return this.channel.loadObjects([{ table: table, keys: keys }]).then((results) => {
      let ret: any[] = []
  ...
  loadOrCreate(table: string, key: string): Promise<DatabaseObject> {
    return this.channel.createObjects([{ key: key, table: table, properties: [] }], true).then((results) => {
      if (results.objects.length == 1)
  ...
  loadKeysOrCreate(table: string, keys: string[]): Promise<DatabaseObject[]> {
    ...
    return this.channel.createObjects(objectIds, true).then((results) => {
  ...
  loadSingle(table: string, index: string, indexValue?: any) {
    return this.channel.loadMatchingObjects(table, index, this.getIndexValue(indexValue), 1).then((results) => {
  ...
  loadRange(
    ...
    stop?: any,
    limit?: number,
  ): Promise<(DatabaseObject | null)[]> {
    return this.channel
      .loadIndexRange(table, index, this.getIndexValue(indexPath, start), this.getIndexValue(indexPath, stop), limit)
      .then((results) => {
  ...
  deleteRange(table: string, index: string, indexPath: any, start: any, stop: any) {
    return this.channel.deleteIndexRange(
      table,
      index,
      this.getIndexValue(indexPath, start),
      this.getIndexValue(indexPath, stop),
    )
  }
    ...
    this.queuedSaves.push({
      ...
      execute: function (channel: HTTPChannel) {
        ...
        if (self.changesets.length > 0) {
          channel.saveObjectChanges(LockType.LockAll, self.changesets, createIfMissing).then((result) => {
        ...
        for (let i in save.objects) {
          for (let f = s + 1; f < this.queuedSaves.length; f++) {
            ...
            for (let o = 0; o < futureSave.objects.length; o++) {
              ...
              if (
                futureSave.objects[o] == save.objects[i] &&
                futureSave.fullOverwrite == save.fullOverwrite &&
                futureSave.useOptimisticLock == save.useOptimisticLock
              ) {
  ...
  protected getIndexValue(values?: any[], extraValue?: any[]) {
    ...
    for (let i = 0; i != values.length; i++) {
      ...
      switch (typeof value) {
        case 'boolean':
          result.push({ valuetype: ValueType.Bool, bool: value })
          break
        case 'string':
          result.push({ valuetype: ValueType.String, string: value })
          break
        case 'number':
    ...
    if (val) {
      switch (val.valuetype || 0) {
        case ValueType.String:
          target[key] = val.string || ''
          break
        case ValueType.Int:
          target[key] = val.int || 0
          break
        case ValueType.UInt:
          target[key] = val.uint || 0
          break
        case ValueType.Long:
          target[key] = val.long || 0
          break
        case ValueType.Bool:
          target[key] = val.bool || 0
          break
        case ValueType.Float:
          target[key] = val.float || 0.0
          break
        case ValueType.Double:
          target[key] = val.double || 0.0
          break
        ...
        case ValueType.DateTime:
          target[key] = new Date(val.datetime || 0)
          break
  ...
  _internal_(method: InternalType, arg1?: any) {
    switch (method) {
      ...
      case 'change-dbCurrent':
        bigDBDeserialize(arg1, this.dbCurrent, true)
    }
  }
}
