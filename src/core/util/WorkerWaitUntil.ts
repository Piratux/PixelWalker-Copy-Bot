...
// Copy of https://github.com/devlato/async-wait-until/blob/master/src/index.ts
// Improved by using worker set timeout to avoid browser throttle of timeouts.
// May switch back to using library if this ever gets implemented:
// https://github.com/devlato/async-wait-until/issues/77
 ...
 * ```
 */
export const workerWaitUntil = <T extends PredicateReturnValue>(
  ...
  return Promise.race([
    ...(timeoutMs !== WAIT_FOREVER
      ? [
          new Promise<never>((_, reject) => {
            timeoutHandle = workerSetTimeout(() => {
    ...
    new Promise<T>((resolve, reject) => {
      const check = async () => {
        try {
          ...
          intervalHandle = workerSetTimeout(check, intervalMs)
        } catch (err) {
          reject(err)
        }
      }
    ...
    timeoutHandle && workerClearTimeout(timeoutHandle)
    intervalHandle && workerClearTimeout(intervalHandle)
  })
}
